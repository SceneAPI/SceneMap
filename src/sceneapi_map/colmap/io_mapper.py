"""``ColmapMapper`` — the neutral :class:`sceneapi_io.mapping.Mapper`
...
:class:`~sceneapi_io.data.CorrespondenceGraph` + views into a sparse
...
1. **graph -> COLMAP database** (:func:`graph_to_colmap_database`): the
   per-image :class:`~sceneapi_io.data.FeatureSet` s become ``keypoints``
   rows, the per-pair :class:`~sceneapi_io.data.PairCorrespondences`
   become ``matches`` + ``two_view_geometries`` rows, and one camera +
   image row is written per view. The ``pair_id`` encoding and schema
   version come from the ``sceneapi_io.colmap_db`` contract (the single
...
2. **sparse model -> MappingResult**
   ...
   :class:`~sceneapi_io.data.TrackedPointCloud` with real track
   observations.
...
import numpy as np
from sceneapi_io.colmap_db import DATABASE_VERSION_NUMBER, image_pair_to_pair_id
...
# match the sceneapi_io.colmap_db contract; the engine reads keypoints +
...
class ColmapMapper:
    """Mixin making a COLMAP backend a neutral ``sceneapi_io`` ``Mapper``.
