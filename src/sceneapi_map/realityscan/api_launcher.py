from __future__ import annotations

import argparse
import os
from collections.abc import Sequence
from pathlib import Path

from .backend import configure_realityscan_environment


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="sfmapi-realityscan-api",
        description="Run sfmapi with the upstream RealityScan CLI backend.",
    )
    parser.add_argument(
        "--realityscan-executable",
        "--realitycapture-executable",
        "--rc-executable",
        "--realityscan",
        "--realitycapture",
        "--rc",
        dest="realityscan_executable",
        default=None,
        help="Path to RealityCapture.exe, RealityScan.exe, or the install directory.",
    )
    parser.add_argument("--host", default=os.environ.get("SFMAPI_HOST", "127.0.0.1"))
    parser.add_argument("--port", type=int, default=int(os.environ.get("SFMAPI_PORT", "8000")))
    parser.add_argument("--reload", action="store_true", help="Enable uvicorn reload mode.")
    parser.add_argument("--log-level", default=os.environ.get("SFMAPI_LOG_LEVEL", "info"))
    parser.add_argument(
        "--mcp",
        choices=("off", "local"),
        default=None,
        help="Set SFMAPI_MCP_MODE for this API process. Use 'local' to mount /mcp.",
    )
    parser.add_argument(
        "--mcp-mount-path",
        default=os.environ.get("SFMAPI_MCP_MOUNT_PATH"),
        help="Root-relative MCP mount path when --mcp local is used.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Configure environment and print the selected RealityScan executable.",
    )
    return parser


def configure_environment(args: argparse.Namespace) -> Path | None:
    os.environ.setdefault("SFMAPI_EPHEMERAL", "true")
    os.environ["SFMAPI_BACKEND"] = "realityscan_cli"
    os.environ.setdefault(
        "SFMAPI_DB_URL", "sqlite+aiosqlite:///file::memory:?cache=shared&uri=true"
    )
    os.environ.setdefault("SFMAPI_BLOB_BACKEND", "memory")
    os.environ.setdefault("SFMAPI_QUEUE_BACKEND", "inline")
    os.environ.setdefault("SFMAPI_INLINE_TASKS", "true")
    if args.mcp is not None:
        os.environ["SFMAPI_MCP_MODE"] = str(args.mcp)
    if args.mcp_mount_path:
        os.environ["SFMAPI_MCP_MOUNT_PATH"] = str(args.mcp_mount_path)
    return configure_realityscan_environment(
        args.realityscan_executable,
        validate=bool(
            args.realityscan_executable
            or os.environ.get("SFMAPI_RC_EXECUTABLE")
            or os.environ.get("SFMAPI_REALITYCAPTURE_EXECUTABLE")
            or os.environ.get("SFMAPI_REALITYSCAN_EXECUTABLE")
        ),
    )


def main(argv: Sequence[str] | None = None) -> None:
    parser = build_parser()
    args = parser.parse_args(argv)
    realityscan_executable = configure_environment(args)
    if args.dry_run:
        selected = (
            str(realityscan_executable) if realityscan_executable else "PATH lookup at runtime"
        )
        print(f"sfmapi backend: realityscan_cli\nReality CLI executable: {selected}")
        return

    import uvicorn

    uvicorn.run(
        "sceneapi_map.realityscan.server:app",
        host=args.host,
        port=args.port,
        reload=args.reload,
        log_level=args.log_level,
    )


if __name__ == "__main__":
    main()
