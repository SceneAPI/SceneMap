"""``MapAnythingBackend`` — the feed-forward :class:`sceneapi_io.mapping.Mapper`
...
neutral sceneapi-io contract with no core routing changes: core's
...
1. **ViewInput -> model view dict** (:func:`build_view_payload`): the image
   becomes ``img`` (H, W, 3); an optional :class:`~sceneapi_io.data.Calibration`
   becomes ``intrinsics`` (a 3x3 K) XOR ``ray_directions`` (H, W, 3); an
   optional :class:`~sceneapi_io.data.PosePrior` becomes ``camera_poses``
   (4x4 cam2world OpenCV) + ``is_metric_scale``; an optional
   :class:`~sceneapi_io.data.DepthMap` prior becomes ``depth_z``.
...
3. **predictions -> MappingResult** (:func:`result_from_predictions`):
   per-view ``camera_poses`` -> cam2world :class:`~sceneapi_io.data.SE3`;
   predicted ``intrinsics`` -> per-view :class:`~sceneapi_io.data.Calibration`;
   ``pts3d`` + ``conf`` -> the dense
   ``(Pointmap, ConfidenceMap)`` payload; the per-view pointmaps fused into a
   subsampled :class:`~sceneapi_io.data.TrackedPointCloud`.
...
def frame_meta(views: Sequence[ViewInput], *, weights: str, options: MappingOptions) -> FrameMeta:
    ...
    - any input view with a metric :class:`~sceneapi_io.data.PosePrior`
...
class MapAnythingBackend:
    """MapAnything feed-forward provider — a neutral sceneapi-io ``Mapper``.

    Implements the minimum sceneapi Backend identity (``name`` / ``version`` /
    ``vendor`` / :meth:`capabilities` / :meth:`runtime_versions`) AND the
    sceneapi-io ``Mapper`` contract (:meth:`traits` / :meth:`map`), so core's
    ...
    def capabilities(self) -> set[str]:
        ...
        absent.
        """
        from sceneapi_io.mapping import Mapper
